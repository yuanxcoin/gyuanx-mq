...
zmq::socket_t& GyuanxMQ::get_control_socket() {
    ...
    if (proxy_shutting_down)
        throw std::runtime_error("Unable to obtain GyuanxMQ control socket: proxy thread is shutting down");
        ...
        sn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{
    ...
    if (pubkey.empty() != privkey.empty()) {
        throw std::invalid_argument("GyuanxMQ construction failed: one (and only one) of pubkey/privkey is empty. Both must be specified, or both empty to generate a key.");
    } else if (pubkey.empty()) {
        if (service_node)
            throw std::invalid_argument("Cannot construct a service node mode GyuanxMQ without a keypair");
        LMQ_LOG(debug, "generating x25519 keypair for remote-only GyuanxMQ instance");
        ...
        if (verify_pubkey != pubkey)
            throw std::invalid_argument("Invalid pubkey/privkey values given to GyuanxMQ construction: pubkey verification failed");
    }
}
...
GyuanxMQ::~GyuanxMQ() {
    ...
    LMQ_LOG(info, "GyuanxMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    LMQ_LOG(info, "GyuanxMQ proxy thread has stopped");
}
